...
def do_search(node, depth):
    """Does one search off a given node (potentially adding children)"""
    return 0.5
...
class Searcher(multiprocessing.Process):
    ...
    def run(self):
        ...
        while True:
            ...
            for _ in xrange(num):
                q_sum += do_search(node, self.tree_depth)
...
class ParallelSearcher(object):
    ...
    def __init__(self, max_depth, num_processes=multiprocessing.cpu_count()*2):
